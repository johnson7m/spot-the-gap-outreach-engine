...
export async function applyMissingNextTaskPlan({
  plan = {},
  config = {},
  options = {},
  restClient,
  ...
  if (liveEnabled && !normalizedOptions.batchSizeProvided) {
    const error = new Error(
      'MISSING_NEXT_TASK_BATCH_SIZE is required for live missing next-task apply.'
    );
    error.code = 'MISSING_NEXT_TASK_BATCH_SIZE_REQUIRED';
    throw error;
  }
  ...
  for (const [index, operation] of operations.entries()) {
    ...
    const auditBase = buildMissingNextTaskCrmSyncLogEntry({
      operation,
      status: 'planned',
      startedAt
    });
    const eventBase = buildMissingNextTaskOutboundEventEntry({
      operation,
      status: 'planned',
      now
    });
...
export function normalizeMissingNextTaskApplyOptions(options = {}) {
...
export function isEligibleMissingNextTaskRecord(record = {}, options = {}) {
  ...
  if (!record.cadenceName || !record.cadenceStage || !record.recommendedTaskType) {
    return false;
  }
...
export function buildMissingNextTaskPayload({ record = {}, dedupeKey, now = new Date() } = {}) {
  const payload = {
    ...
    bodyV2: {
      markdown: buildMissingNextTaskMarkdown({
        record,
        dedupeKey,
        now
      })
...
function getOperationSkippedReason({ record, taskPayload }) {
  ...
  if (!record.cadenceName || !record.cadenceStage || !record.recommendedTaskType) {
    return 'Missing cadence or task type context.';
  }
...
async function verifyMissingNextTask({ client, operation, taskId }) {
  const [task, taskTargets] = await Promise.all([
  ...
  return {
    ok: Boolean(task?.id) && Boolean(personTarget) && (!operation.companyTargetEnabled || Boolean(companyTarget)),
...
async function findOpenTaskForPerson({ client, personId }) {
  const [tasks, taskTargets] = await Promise.all([
    listAllRecords(client, 'tasks'),
    listAllRecords(client, 'taskTargets')
  ]);
...
function buildMissingNextTaskCrmSyncLogEntry({ operation, status, startedAt }) {
  return {
...
function buildMissingNextTaskOutboundEventEntry({ operation, status, now }) {
  return {
    assessmentSubmissionId: null,
    correlationId: operation.correlationId,
...
function summarizeMissingNextTaskOperationResults(summary) {
  return {
...
function buildMissingNextTaskMarkdown({ record = {}, dedupeKey, now = new Date() }) {
  return [
