...
- Fresh Leads own first-touch work: `NOT_STARTED` or `CONNECTION_REQUEST`
  records with initial connection/request Tasks.
...
- A `NOT_STARTED` Person with `Send relationship-oriented connection request`
  or `Send assessment-oriented connection request` stays in Fresh Leads and is
